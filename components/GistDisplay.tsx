...
export default function GistDisplay({ gist }: { gist: Gist }) {
  ...
  return (
    <div className="card">
      {/* Fork 来源提示 */}
      {gist.forked_id && (
        <div className="px-4 py-2 border-b text-sm" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-bg-subtle)', color: 'var(--color-text-secondary)' }}>
          ...
          Forked from <Link href={`/gists/${gist.forked_id}`} className="font-medium hover:underline" style={{ color: 'var(--color-accent)' }}>another gist</Link>
        </div>
      )}
        ...
        <div className="flex border-b overflow-x-auto" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-bg-code)' }}>
          {gist.files.map((file, index) => (
            <button
              ...
              style={{
                color: activeFile === index ? 'var(--color-text)' : 'var(--color-text-secondary)',
                borderColor: activeFile === index ? 'var(--color-accent)' : 'transparent'
              }}
            >
